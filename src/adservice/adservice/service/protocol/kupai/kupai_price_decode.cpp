...
int64_t kupai_price_decode(const std::string & input)
{
    try {
        ...
        LOG_ERROR << "kupai_price_decode price error,input:" << input << ",e:" << pe.trace();
    } catch (std::exception & e) {
        LOG_ERROR << "kupai_price_decode price error,input:" << input << ",e:" << e.what();
    }
    return 0;
}
