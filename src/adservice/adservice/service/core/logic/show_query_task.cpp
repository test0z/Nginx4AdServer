...
int buildResponseForDsp(const adselectv2::AdBanner& banner,ParamMap& paramMap,const char* json,const char* templateFmt,char* buffer,int bufferSize){
    char pjson[2048]={'\0'};
...
int buildResponseForSsp(const adselectv2::AdSelectResult& selectResult,ParamMap& paramMap,
                        const char* json,const char* templateFmt,char* buffer,int bufferSize,const std::string& userIp,
                        const std::string& referer){
    const adselectv2::AdSolution& solution = selectResult.solution;
    const adselectv2::AdBanner& banner = selectResult.banner;
    const adselectv2::AdAdplace& adplace = selectResult.adplace;
    char pjson[2048]={'\0'};
    strncat(pjson,json,sizeof(pjson));
    tripslash2(pjson);
    cppcms::json::value mtAdInfo;
    utility::json::parseJson(pjson,mtAdInfo);
    std::string pid = to_string(adplace.adplaceId);
    mtAdInfo["pid"] = pid;
    mtAdInfo["adxpid"] = pid;
    ...
    mtAdInfo["unid"] = adxid;
    mtAdInfo["plid"] = "";
    std::string sid = to_string(solution.solutionId);
    mtAdInfo["gpid"] = sid;
    ...
    if(paramMap[URL_IMP_OF] == OF_SSP_MOBILE || pid=="3381") {
        ...
        size_t clickMacroLen = (size_t)snprintf(clickMacroBuffer, sizeof(clickMacroBuffer),
...
void HandleShowQueryTask::customLogic(ParamMap & paramMap, protocol::log::LogItem & log, adservice::utility::HttpResponse & response)
{
    ...
    if(isSSP){//SSP
        ...
        adselectv2::AdSelectResponse selectResp;
        if(!adSelectClient->doRequest(seqId,true,condition,selectResp)){
            LOG_DEBUG<<"ssp module:adselect failed";
            const adselectv2::AdAdplace& tmpadplace = selectResp.result.adplace;
            log.adInfo.adxid = condition.adxid;
            log.adInfo.pid = std::to_string(tmpadplace.adplaceId);
            log.adInfo.adxpid = tmpadplace.adxPid;
            log.reqStatus = 500;
            return;
        }
        const adselectv2::AdSelectResult& selectResult = selectResp.result;
        const adselectv2::AdBanner& banner = selectResult.banner;
        const adselectv2::AdSolution& finalSolution = selectResult.solution;
        const adselectv2::AdAdplace& adplace = selectResult.adplace;
        log.adInfo.bannerId = banner.bannerId;
        log.adInfo.advId = finalSolution.advId;
        log.adInfo.adxid = adplace.adxId;
        log.adInfo.pid = to_string(adplace.adplaceId);
        ...
        //返回结果
        char buffer[8192];
        int len = buildResponseForSsp(selectResult,paramMap,tmp,templateFmt,buffer,sizeof(buffer),userIp,referer);
        respBody = std::string(buffer,buffer+len);
    }else {//DSP of=0,of=2,of=3
        adselectv2::AdDataManager::AdDataManagerPtr& dataManager = AdDataManager::getInstance();
        bool showCreative = true;
        dspSetParam(paramMap,showCreative,needLog);
        adselectv2::AdBanner adBanner;
        int bannerId = std::stoi(paramMap[URL_CREATIVE_ID]);
        if (!dataManager->getBanner(bannerId,adBanner)) {
    ...
    response.set_header("P3p", "CP=\"CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR\"");
    handleShowRequests++;
