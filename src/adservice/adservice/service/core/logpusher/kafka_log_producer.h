...
namespace log {
    ...
    public:
        KafkaLogProducer(const std::string & name, const std::string & configKey = CONFIG_LOG)
            : myName(name)
            , logConfigKey(configKey)
        {
            configure();
        }
        ~KafkaLogProducer()
        ...
        void setDeliverReportCallback(const LogDeliverReportCb && cb)
        {
            drCb = cb;
        }
        void setErrorCallback(const LogEventCb && cb)
        {
            eventCb = cb;
        }
