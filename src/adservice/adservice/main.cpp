...
#include "core/adselectv2/addatamanager.h"
...
struct LocationConf {
    ...
    //adselect 是否使用unix socket
    ngx_uint_t adselectuseunixsocket;
    //如果使用unix socket,sock file文件路径
    ngx_str_t adselectunixsocketfile;
    //addata 配置
    //addata namespace
...
#define COMMAND_ITEM(name,configstruct,variable,parser) {\
    ...
    offsetof(configstruct,variable),\
    0\
}
...
static ngx_command_t commands[] = {
    ...
    COMMAND_ITEM("adselect_use_unix_socket",LocationConf,adselectuseunixsocket,parseConfNum),
    COMMAND_ITEM("adselect_unix_socket_file",LocationConf,adselectunixsocketfile,parseConfStr),
...
#define INIT_NGX_STR(str,initvalue) {str.data = (u_char*)initvalue,str.len=strlen(initvalue);}
#define TRUE    1
#define FALSE   0
...
static void * createLocationConf(ngx_conf_t * cf)
{
    ...
    conf->adselectuseunixsocket = NGX_CONF_UNSET_UINT;
    ngx_str_null(&conf->adselectunixsocketfile);
...
static char * mergeLocationConf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_uint_value(conf->adselectuseunixsocket,prev->adselectuseunixsocket,FALSE);
    ngx_conf_merge_str_value(conf->adselectunixsocketfile,prev->adselectunixsocketfile,"/tmp/adselect.sock");
...
void parseConfigAeroSpikeNode(const std::string& asNode,AerospikeConfig& config){
    ...
    while((nextPos = asNode.find(",",prevPos))!=std::string::npos){
        ...
        if((portPos=asNode.find(":",prevPos))!=std::string::npos){
            config.connections.push_back(AerospikeConfig::Connection(std::string(asNode.data()+prevPos,asNode.data()+portPos),std::stoi(asNode.data()+portPos+1)));
        }else{
            config.connections.push_back(AerospikeConfig::Connection(std::string(asNode.data()+prevPos,asNode.data()+nextPos),3000));
        }
        prevPos = nextPos+1;
    }
}
...
static void global_init(LocationConf* conf){
    ...
    globalConfig.adselectConfig.useUnixSocket = NGX_BOOL(conf->adselectuseunixsocket);
    globalConfig.adselectConfig.unixSocketFile = NGX_STR_2_STD_STR(conf->adselectunixsocketfile);
    ...
    serviceLogger = adservice::log::LogPusher::getLogger(
            MTTY_SERVICE_LOGGER, CONFIG_LOG, globalConfig.logConfig.localLoggerThreads, !globalConfig.logConfig.kafkaLogEnable);
    serviceLogger->start();

    adSelectClient = std::shared_ptr<adservice::adselectv2::AdSelectClient>(new adservice::adselectv2::AdSelectClient(globalConfig.adselectConfig.adselectNode,
                                     globalConfig.adselectConfig.adselectTimeout,
                                     globalConfig.adselectConfig.useUnixSocket,
                                     globalConfig.adselectConfig.unixSocketFile));
    adservice::adselectv2::AdDataManager::instance =
    std::shared_ptr<adservice::adselectv2::AdDataManager>(new adservice::adselectv2::AdDataManager(globalConfig.addataConfig));
